...
//global struct 
struct AIR_arameters_structure
{
  uint8_t temp_int;
  uint8_t temp_dec;
  uint8_t humid_int;
...
struct Timer_arameters_structure
{
  uint8_t time_sec;
  uint8_t time_minute;
  uint8_t time_hour;
...
ISR(TIMER1_OVF_vect)
{
    ...
    if(ack == 0)
    {
      ...
      twi_stop();
      //hour
      itoa(time.time_hour, string, 16);
      uart_puts("Time: ");
      uart_puts(string);
      uart_puts(":");
      //min
      itoa(time.time_minute, string, 16);
      uart_puts(string);
      uart_puts(":");
      //sec
      itoa(time.time_sec, string, 16);
      uart_puts(string);
      uart_puts("\r\n");
    }
  /*  if(sla <120)
